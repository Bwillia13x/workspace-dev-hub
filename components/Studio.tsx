...
import { generateConcept, editConcept, generateEngineeringPack, getFashionTrends } from '../services/geminiService';
...
export const Studio: React.FC<StudioProps> = ({ onPublish, onShowToast, initialDraft, readOnly = false }) => {
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setLoadingMessage('Designing your vision with Gemini 2.5...');
    try {
      const base64 = await generateConcept(prompt);
      setDraft(prev => ({
        ...prev,
        conceptImage: base64,
        cadImage: null,
  ...
  const handleTrendSearch = async () => {
    ...
    setTrendResult(null);
    try {
        const result = await getFashionTrends(trendQuery);
  ...
  const handleEdit = async () => {
    ...
    try {
      const base64 = await editConcept(draft.conceptImage, editPrompt);
      setDraft(prev => ({
        ...prev,
        conceptImage: base64,
  ...
  const handleEngineer = async () => {
    ...
    try {
      const { cadImage, materials } = await generateEngineeringPack(draft.conceptImage);
      setDraft(prev => ({
        ...prev,
        cadImage,
        materials
           ...
           <div className="animate-fade-in">
             ...
             <div className="group relative mb-5">
               ...
               <div className={`relative bg-[#0F131E] border border-white/10 rounded-xl p-1 shadow-inner ${isReadOnly ? 'opacity-50' : ''}`}>
                 ...
                 />
                 <div className="flex justify-end p-2 border-t border-white/5 bg-[#0F131E] rounded-b-lg">
                    {isReadOnly ? (
             ...
             <div className={`absolute top-6 left-1/2 -translate-x-1/2 z-50 animate-slide-up ${isReadOnly ? 'mt-10' : ''}`}>
                 <div className="bg-[#0F131E]/80 backdrop-blur-xl border border-white/10 rounded-full px-4 py-2 flex items-center gap-4 shadow-2xl">
                    ...
                    <div className="flex items-center gap-2 border-r border-white/10 pr-4">
                        <button onClick={() => setZoomLevel(Math.max(0.5, zoomLevel - 0.1))} className="text-slate-400 hover:text-white transition-colors">
                        ...
                        <button onClick={() => setZoomLevel(Math.min(2, zoomLevel + 0.1))} className="text-slate-400 hover:text-white transition-colors">
